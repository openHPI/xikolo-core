...
describe AccountService::User::ReplaceEmails, type: :operation do
  ...
  describe 'with a valid email address' do
    ...
    it 'deletes existing email records for the user' do
      expect { operation.call }.to change { AccountService::Email.exists?(id: [primary_email, confirmed_email, unconfirmed_email]) }.from(true).to(false)
    end
  ...
  describe 'with multiple valid email addresses' do
    ...
    it 'creates new email records, and returns them with the correct attributes' do
      ...
      expect(new_primary).to be_a AccountService::Email
      ...
      expect(new_confirmed).to be_a AccountService::Email
      ...
      expect(new_unconfirmed).to be_a AccountService::Email
  ...
  shared_examples_for 'an invalid email address replacement operation' do |error|
    it 'does not delete existing email records' do
      expect { operation.call }.to raise_error(error) do
        expect(AccountService::Email.exists?(primary_email.id)).to be(true)
        expect(AccountService::Email.exists?(confirmed_email.id)).to be(true)
        expect(AccountService::Email.exists?(unconfirmed_email.id)).to be(true)
      end
    end
  end
  ...
  describe 'with an empty email list' do
    ...
    it_behaves_like 'an invalid email address replacement operation', AccountService::User::ReplaceEmails::EmptyEmailsError
  end
  ...
  describe 'with a confirmed, but not primary email address' do
    ...
    it_behaves_like 'an invalid email address replacement operation', AccountService::User::ReplaceEmails::InvalidEmailsError
  end
