...
describe API::AuthorizationsController, type: :controller do
  let(:record) { create(:authorization) }
  let(:records) { create_list(:authorization, 10, expires_at: Time.zone.now) }
  ...
  describe '#index' do
    ...
    context 'default' do
      ...
      it 'contain authorization objects' do
        expect(json).to match_array AuthorizationDecorator
  ...
  describe '#update' do
    ...
    it 'invokes authorization provider' do
      expect(Xikolo::Provider).to receive(:update).with(record)
      response
    end
  end
  ...
  describe '#create' do
    ...
    let(:params) { attributes_for(:authorization, info: {'hash' => 'true'}) }

    it 'creates new authorization record' do
      expect { response }.to change(Authorization, :count).from(0).to(1)
    end

    describe 'created record' do
      subject(:record) { response; Authorization.last }
