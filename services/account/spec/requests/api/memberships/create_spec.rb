...
describe 'Memberships: Creation', type: :request do
  ...
  shared_examples 'membership creation' do
    ...
    it 'responds with a follow location to created membership' do
      expect(resource.follow).to eq account_service.membership_url(AccountService::Membership.last)
    end

    it 'creates a database record' do
      expect { resource }.to change(AccountService::Membership, :count).from(0).to(1)

      expect(AccountService::Membership.last.group).to eq AccountService::Group.find_by! name: data[:group]
      expect(AccountService::Membership.last.user).to eq user
    end

    it 'responses with a membership resource' do
      expect(resource).to eq json(AccountService::Membership.last)
    end
  end
  ...
  context 'with existing group' do
    ...
    context 'with existing membership' do
      ...
      it 'does not create a second membership' do
        expect { resource }.not_to change(AccountService::Membership, :count).from(1)
      end
