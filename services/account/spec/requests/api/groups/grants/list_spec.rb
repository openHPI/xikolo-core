...
describe 'Groups: Grants: Listing', type: :request do
  ...
  let!(:grants) do
    [
      create(:'account_service/grant', principal: group, role: roles[0], context: Context.root),
      create(:'account_service/grant', principal: group, role: roles[1], context:),
    ]
  end

  before do
    create(:'account_service/grant', principal: create(:'account_service/group'), role: roles[0], context: Context.root)
  end
