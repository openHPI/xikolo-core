...
describe 'Learning Evaluation: Update course progress', type: :feature do
  ...
  describe 'CourseService::Creating a result' do
    ...
    it 'does not immediately create a new course progress' do
      expect { create_result }.not_to change(CourseService::CourseProgress, :count)
    end
    ...
    it '(asynchronously) creates a new course progress' do
      ...
      end.to change(CourseService::CourseProgress, :count).by(1)

      expect(CourseService::CourseProgress.last).to have_attributes(
        main_dpoints: 10,
        main_exercises: 1,
        bonus_dpoints: 0,
        bonus_exercises: 0,
    ...
    it '(asynchronously) updates an existing course progress' do
      ...
      expect do
        Sidekiq::Testing.inline! { create_result }
      end.not_to change(CourseService::CourseProgress, :count)
  ...
  describe 'Updating a result' do
    ...
    it 'does not immediately create a new course progress' do
      expect { update_result }.not_to change(CourseService::CourseProgress, :count)
    end
  ...
  describe 'CourseService::Creating a visit' do
    ...
    it 'does not immediately create a new course progress' do
      expect { create_visit }.not_to change(CourseService::CourseProgress, :count)
    end
    ...
    it '(asynchronously) creates a new course progress' do
      ...
      expect(CourseService::CourseProgress.last).to have_attributes(visits: 1)
    end
