  ...
  context 'with valid upload' do
    let!(:store_stub) do
      stub_request(:put, %r{https://s3.xikolo.de/xikolo-public/
        channels/[0-9a-zA-Z]+/stage_visual_v2.jpg}x).to_return(status: 200, body: '<xml></xml>')
    end
    ...
    before do
      ...
      stub_request(:head, file_url).to_return(
        ...
        headers: {
          ...
          'X-Amz-Meta-Xikolo-Purpose' => 'course_channel_stage_visual',
    ...
    it 'stores the stage visual' do
      expect { action }.to change { channel.reload.stage_visual_uri }
  ...
  context 'with invalid file upload' do
    ...
    it 'returns an error on inprocessible upload' do
      ...
      expect { action }.to raise_error(Restify::ClientError) do |error|
        ...
        expect(error.errors).to eq 'stage_visual_upload_id' => ['invalid upload']
      end
    end
    ...
    it 'handles S3 errors during upload validating' do
      ...
      expect { action }.to raise_error(Restify::ClientError) do |error|
        ...
        expect(error.errors).to eq \
          'stage_visual_upload_id' => ['could not process file upload']
      end
    end
    ...
    it 'handles S3 errors during upload copying' do
      ...
      stub_request(:put, %r{https://s3.xikolo.de/xikolo-public/
        channels/[0-9a-zA-Z]+/stage_visual_v2.jpg}x).to_return(status: 403)
