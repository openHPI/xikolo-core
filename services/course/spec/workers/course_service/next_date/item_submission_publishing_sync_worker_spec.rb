...
RSpec.describe CourseService::NextDate::ItemSubmissionPublishingSyncWorker, type: :worker do
  ...
  it 'updates attributes (like title) of all created next dates' do
    item
    Sidekiq::Testing.inline! do
      CourseService::NextDate::ItemSubmissionDeadlineSyncWorker.perform_async(item.id)
    end
    # Expect a NextDate for the item's submission deadline to exist
    expect(CourseService::NextDate.count).to eq 1
    user_ids.map do |user_id|
      CourseService::NextDate.create! attrs.merge(title: 'Test', user_id:, date: 2.days.from_now)
    end
    expect(CourseService::NextDate.count).to eq 6
    expect(CourseService::NextDate.pluck(:title)).to include('Test')
    ...
    expect(CourseService::NextDate.count).to eq 6
    expect(CourseService::NextDate.pluck(:title)).not_to include('Test')
  end
  ...
  context 'for item with submission_deadline' do
    ...
    it 'updates the NextDates but does not remove any' do
      expect { execute }.not_to change(CourseService::NextDate, :count)
    end
  ...
  context 'for item without submission_deadline and submission_publishing_date' do
    ...
    it 'removes the next dates' do
      expect { execute }.to change(CourseService::NextDate, :count).from(5).to(0)
    end
  end
end
