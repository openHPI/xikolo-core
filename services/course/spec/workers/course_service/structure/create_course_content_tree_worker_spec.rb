...
RSpec.describe CourseService::Structure::CreateCourseContentTreeWorker, type: :worker do
  ...
  it 'creates the course node' do
    expect { perform }.to change { CourseService::Structure::Root.where(course:).count }.from(0).to(1)
  end
  ...
  context 'with sections and items' do
    ...
    it 'creates the section nodes' do
      expect { perform }.to change { CourseService::Structure::Section.where(course:).count }.from(0).to(2)
    ...
    it 'creates item nodes for the respective sections' do
      expect { perform }.to change(CourseService::Structure::Item, :count).from(0).to(5)

      # Ensure the item nodes are assigned to the correct section nodes
      # and have the correct position within the tree.
      items_s1 = CourseService::Structure::Item.where(course:, parent: s1.node.id).order('lft')
      expect(items_s1.map(&:item).pluck(:title)).to eq %w[1.0 1.1]
      expect(s1.node.children.pluck(:item_id)).to eq s1.items.pluck(:id)

      items_s2 = CourseService::Structure::Item.where(course:, parent: s2.node.id).order('lft')
    ...
    it 'does not affect other courses / their structure nodes' do
      ...
      expect { perform }.not_to change { CourseService::Structure::Root.where(course: another_course).count }
    end
  end
end
