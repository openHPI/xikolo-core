...
describe CourseService::SectionChoicesController, type: :controller do
  ...
  describe "GET 'index'" do
    ...
    it 'answers with section resources' do
      get :index
      expect(json[0]).to eq CourseService::SectionChoiceDecorator.new(section_choice).as_json(api_version: 1).stringify_keys
    end
  ...
  describe "POST 'create'" do
    ...
    it 'creates a section_choice on create' do
      post(:create, params:)
      expect(CourseService::SectionChoice.count).to eq(2)
    end

    it 'answers with section_choice' do
      post(:create, params:)
      choice = CourseService::SectionChoice.find_by(user_id: new_user_id, section_id: section.id)
