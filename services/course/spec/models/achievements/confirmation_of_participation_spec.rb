...
describe Achievements::ConfirmationOfParticipation, type: :model do
  variant 'With dynamic learning evaluation' do
    let(:evaluation) do
      LearningEvaluation::Dynamic.new.call(
        Enrollment.where(course:, user_id:)
      ).take
    end
  end
  ...
  variant 'With persisted learning evaluation' do
    let(:evaluation) do
      CourseProgress.find_by(course:, user_id:)
    end
  end
