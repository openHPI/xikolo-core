...
describe Teacher do
  ...
  describe 'callbacks' do
    context 'with courses present' do
      ...
      context 'when name is updated' do
        it 'schedules the update of search index for each course' do
          expect { teacher.update!(name: 'John John') }.to change { UpdateCourseSearchIndexWorker.jobs.count }.by(2)
        end
      end

      context 'when other attributes than name are updated' do
        it 'does not schedule the update of search index' do
          expect { teacher.update!(description: {'de' => 'Toller Lehrer', 'en' => 'A great teacher'}) }.not_to change { UpdateCourseSearchIndexWorker.jobs.count }
        end
      end

      context 'when teacher is destroyed' do
        it 'schedules the worker to update related courses' do
          expect { teacher.destroy }.to change { UpdateCourseSearchIndexWorker.jobs.count }.by(2)
        end
      end
    end
    ...
    context 'without courses' do
      it 'does not schedule the update of search index' do
        expect { teacher.update!(name: 'John Doe') }.not_to change { UpdateCourseSearchIndexWorker.jobs.count }
      end
    end
  end
end
