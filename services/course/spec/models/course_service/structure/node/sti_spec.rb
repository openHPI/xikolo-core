...
describe 'Structure::Node: Single Table Inheritance', type: :model do
  ...
  it "stores a symbolic name to identify a node's concrete type" do
    node = CourseService::Structure::Root.create!(course:)

    expect(node.type).to eq 'root'
  end

  it 'can look up models based on symbolic names' do
    node = CourseService::Structure::Node.create!(type: 'root', course:)

    typed_node = CourseService::Structure::Node.find node.id
    expect(typed_node).to be_a CourseService::Structure::Root
  end
  ...
  it 'can not be looked up with a concrete type that does not match' do
    ...
    expect do
      CourseService::Structure::Section.find node.id
