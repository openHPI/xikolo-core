...
describe LtiExercise::Clone do
  ...
  context 'with a referenced LTI provider' do
    ...
    it 'clones its provider' do
      expect { new_lti_exercise }.to change(Duplicated::LtiExercise, :count).from(1).to(2)
      new_provider = Duplicated::LtiProvider.find(new_lti_exercise.lti_provider_id)
