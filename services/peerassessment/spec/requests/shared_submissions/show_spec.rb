...
describe 'Shared Submission: Show', type: :request do
  subject(:show) do
    test_api.rel(:shared_submission).get(id: shared.id).value!
  end
  ...
  context 'with non-existing peer assessment' do
    it 'returns with 404 not found' do
      expect { test_api.rel(:shared_submission).get(id: SecureRandom.uuid).value! }.to \
