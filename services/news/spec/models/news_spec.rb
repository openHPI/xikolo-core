...
describe News, type: :model do
  ...
  let(:attributes) { {} }
  let(:news) { create(:news, attributes) }

  it { is_expected.to be_valid }

  describe 'validations' do
    context 'global announcements' do
      let(:news) { create(:news, :global) }
  ...
  describe 'scopes' do
    describe '#for_groups' do
      ...
      let!(:global_announcement) { create(:news, :global) }
      let!(:matching_restricted_announcement) { create(:news, :global, audience: 'matching_group') }
      let!(:other_restricted_announcement) { create(:news, :global, audience: 'other_group') }
  ...
  describe '#translated_titles' do
    ...
    context 'with a German translation' do
      let(:news) { create(:news, :with_german_translation) }
    ...
    context 'with visual URI' do
      ...
      it {
        expect(visual_url).to eq \
          'http://s3.xikolo.de/xikolo-public/news/image.png'
      }
    end
  end
end
