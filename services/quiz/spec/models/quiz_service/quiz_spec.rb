...
describe QuizService::Quiz, type: :model do
  subject(:quiz) do
  ...
  describe '#attempt!' do
    ...
    context 'when there is a submitted submission for the user' do
      let!(:existing_submission) { create(:'quiz_service/quiz_submission', :submitted, quiz:, user_id:) }
      ...
      context 'for a quiz with unlimited attempts' do
        ...
        it 'returns a new quiz submission' do
          expect(attempt).to be_a QuizService::QuizSubmission
      ...
      context 'for a quiz with just one attempt' do
        ...
        context 'when the user was granted an additional attempt' do
          before { create(:'quiz_service/additional_quiz_attempt', quiz_id: quiz.id, user_id:) }
    ...
    context 'when the user has an unsubmitted submission that can still be submitted' do
      let!(:existing_submission) { create(:'quiz_service/quiz_submission', quiz:, user_id:, created_at: 1.hour.ago) }
  ...
  describe 'with versioning', :versioning do
    subject do
      QuizService::Quiz.first.paper_trail.version_at DateTime.new(2009, 1, 1, 12, 0, 0).to_s
    end
