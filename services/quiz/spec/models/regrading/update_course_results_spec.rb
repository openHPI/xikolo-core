...
describe Regrading::UpdateCourseResults, type: :model do
  ...
  let!(:answer) { create(:free_text_answer, question:, text: 'foo') }

  let!(:correct_submission) do
    create(:quiz_submission, :submitted, quiz:).tap do |submission|
      create(:quiz_submission_question, quiz_submission: submission, quiz_question_id: question.id, points: nil).tap do |submission_question|
        create(:quiz_submission_free_text_answer, quiz_answer_id: answer.id, quiz_submission_question: submission_question, user_answer_text: 'foo')
      end
    end
  end
  ...
  let!(:wrong_submission) do
    create(:quiz_submission, :submitted, quiz:).tap do |submission|
      create(:quiz_submission_question, quiz_submission: submission, quiz_question_id: question.id, points: nil).tap do |submission_question|
        create(:quiz_submission_free_text_answer, quiz_answer_id: answer.id, quiz_submission_question: submission_question, user_answer_text: 'bar')
      end
    end
  end
