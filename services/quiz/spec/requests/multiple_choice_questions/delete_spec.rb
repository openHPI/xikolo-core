...
RSpec.describe 'Multiple Choice Questions: Delete', type: :request do
  subject(:deletion) { api.rel(:multiple_choice_question).delete({id: question.id}).value! }
  ...
  it 'removes the question' do
    expect { deletion }.to change(Question, :count).from(1).to(0)
  end
