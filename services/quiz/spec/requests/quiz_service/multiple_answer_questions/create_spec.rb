...
RSpec.describe 'Multiple Answer Questions: Create', type: :request do
  ...
  let(:payload) { attributes_for(:'quiz_service/multiple_answer_question', quiz_id: quiz.id) }
  ...
  it 'creates a new multiple answer question' do
    expect { resource }.to change(QuizService::MultipleAnswerQuestion, :count).from(0).to(1)
  end
