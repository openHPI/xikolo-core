...
RSpec.describe 'Answers: Create', type: :request do
  ...
  let(:payload) { attributes_for(:'quiz_service/answer').merge question_id: question.id }
  ...
  it 'creates a new answer' do
    expect { creation }.to change(QuizService::Answer, :count).from(0).to(1)
    expect(QuizService::Answer.last.question).not_to be_nil
  end
  ...
  context 'text with file upload references' do
    ...
    it 'stores valid upload and creates a new resource' do
      ...
      expect { creation }.to change(QuizService::Answer, :count).from(0).to(1)
      expect(QuizService::Answer.last.text).to include 's3://xikolo-quiz/quizzes'
    end
