...
describe QuizService::QuizSubmissionSnapshotsController, type: :controller do
  include_context 'quiz_service API controller'

  let(:quiz_submission_snapshot) { create(:'quiz_service/quiz_submission_snapshot') }
  ...
  describe '#index' do
    ...
    context 'should answer with quiz_submission_snapshot objects' do
      ...
      describe 'response' do
        ...
        its(:first) { is_expected.to eq QuizService::QuizSubmissionSnapshotDecorator.new(quiz_submission_snapshot).as_json(api_version: 1) }
      end
    end
  end
  ...
  describe '#show' do
    ...
    it 'answers with quiz_submission_snapshot object' do
      get :show, params: {id: quiz_submission_snapshot.id}
      expect(json).to eq(QuizService::QuizSubmissionSnapshotDecorator.new(quiz_submission_snapshot).as_json(api_version: 1).stringify_keys)
    end
  end
  ...
  describe '#destroy' do
    ...
    it 'removes a quiz_submission_snapshot' do
      ...
      expect { action }.to change(QuizService::QuizSubmissionSnapshot, :count).from(1).to(0)
    end
  end
end
