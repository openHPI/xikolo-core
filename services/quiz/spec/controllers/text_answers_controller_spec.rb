...
describe TextAnswersController, type: :controller do
  let(:answer) { create(:text_answer) }
  let(:params) { attributes_for(:text_answer) }
  ...
  describe '#create' do
    ...
    context 'FreeTextAnswer' do
      ...
      let(:params) { attributes_for(:answer).merge(correct: false) }
  ...
  describe '#destroy' do
    ...
    let!(:answer) { create(:text_answer, text: 's3://xikolo-quiz/quizzes/1/1/test.jpg') }
  ...
  describe 'with versioning', :versioning do
    let(:answer1) { create(:text_answer, correct: true, comment: 'Correct answer, well done!') }
    let(:answer2) { create(:text_answer, correct: true) }
    let(:answer3) { create(:text_answer, correct: false) }
