...
describe UserQuizAttemptsController, type: :controller do
  let!(:quiz_submission) { create(:quiz_submission, :submitted) }
  ...
  describe '#show' do
    ...
    context 'with valid params' do
      let(:params) do
        {user_id: quiz_submission.user_id, quiz_id: quiz_submission.quiz_id}
      end
  ...
  describe '#create' do
    ...
    it 'creates a new quiz attempt' do
      expect { request }.to change(AdditionalQuizAttempt, :count).from(0).to(1)
    end
    ...
    context 'with existing additional quiz attempts' do
      let!(:existing_attempts) do
        create(:additional_quiz_attempt,
          user_id: quiz_submission.user_id,
          quiz_id: quiz_submission.quiz_id)
      end
      ...
      it 'does not create a new quiz attempt' do
        expect { request }.not_to change(AdditionalQuizAttempt, :count)
      end
    ...
    context 'with course ID' do
      ...
      it 'schedules a job to unlock the assignments' do
        expect { request }.to change(UnlockCourseAssignmentsWorker.jobs, :size).by(1)
      end
