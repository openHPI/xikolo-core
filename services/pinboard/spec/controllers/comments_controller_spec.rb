...
describe CommentsController, type: :controller do
  ...
  describe "GET 'index'" do
    ...
    it 'answers with comment resources' do
      expect(json.first).to eq(CommentDecorator.new(comment).as_json.stringify_keys)
    end
  ...
  describe "GET 'show'" do
    ...
    it 'answers with a comment resource' do
      expect(json).to eq(CommentDecorator.new(comment).as_json.stringify_keys)
    end
    ...
    context 'with a question' do
      ...
      context 'with image references' do
        ...
        it 'stores valid upload and creates a new richtext' do
          ...
          expect { response }.to change(Comment, :count).by(1)
          expect(Comment.find(json['id']).text).to include 's3://xikolo-pinboard'
        end
      ...
      context 'with notification' do
        ...
        it 'creates subscription with answer' do
          expect { response }.to change(Subscription, :count).from(0).to(1)
        end
      end
    end
