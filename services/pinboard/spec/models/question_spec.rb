...
describe Question, type: :model do
  ...
  context '(subscriptions)' do
    ...
    it 'deletes associated subscriptions when removed' do
      expect do
        question.destroy!
      end.to change(Subscription, :count).from(3).to(0)
    end
  end
  ...
  describe 'sorting' do
    ...
    context 'by sum of votes, same votes chronologically' do
      ...
      context 'descending' do
        subject { Question.default_order.order_by_votes(:desc).pluck :id }
    ...
    context 'by sticky flag' do
      subject { Question.default_order.pluck :id }
  ...
  describe '.by_tags' do
    ...
    it 'retrieves the tagged question when using the right tag' do
      ...
      expect(Question.by_tags([tag_id])).to eq [question_with_tags]
    end

    it 'retrieves the questions fitting all tags' do
      tags = question_with_tags.tags
      expect(Question.by_tags(tags.map(&:id))).to eq [question_with_tags]
    end

    it 'returns nothing for an unused tag' do
      tag = Tag.create course_id: '00000001-3300-4444-9999-000000000002', name: 'Foo'
      expect(Question.by_tags([tag.id])).to eq []
    end
  end
  ...
  describe '#soft_delete' do
    ...
    it 'does not delete the question' do
      expect { action.call }.not_to change(Question, :count)
    end
