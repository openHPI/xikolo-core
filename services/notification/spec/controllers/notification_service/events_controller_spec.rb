...
describe NotificationService::EventsController, type: :controller do
  ...
  describe 'GET index' do
    ...
    context 'for a user' do
      let!(:event) { create(:'notification_service/event', :with_notifications, notify_user: [user_id]) }
      let(:params) { super().merge(user_id:) }
      let(:user_id) { generate(:user_id) }

      context 'with public, non-public, and events with notifications' do
        let!(:public_events) { create_list(:'notification_service/event', 5) }
        let!(:notification_events) { create_list(:'notification_service/event', 5, :with_notifications, notify_user: [user_id, generate(:user_id)]) }

        before do
          # Five more private events that should not be listed
          create_list(:'notification_service/event', 5, :not_public)
        end
    ...
    context 'with course_id' do
      ...
      let(:event) { create(:'notification_service/event', course_id:) }

      before do
        # Another event in a different course
        create(:'notification_service/event', course_id: course_id2)
      end
