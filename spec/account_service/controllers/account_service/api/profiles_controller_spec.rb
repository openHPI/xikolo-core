...
require 'spec_helper'
require 'account_service/xikolo_validators/base'

describe AccountService::API::ProfilesController, type: :controller do
  ...
  describe '#show' do
    ...
    before do
      create(:'account_service/custom_field_value',
  ...
  describe 'PATCH update' do
    ...
    context 'with text field' do
      let!(:field) { create(:'account_service/custom_text_field') }
      ...
      context 'with valid string' do
        ...
        it 'creates custom field value' do
          expect { response }.to change(AccountService::CustomFieldValue, :count).from(0).to(1)

          AccountService::CustomFieldValue.first.tap do |value|
      ...
      shared_examples_for 'a blank value' do |values|
        ...
        it 'does not create a custom field value' do
          expect { response }.not_to change(AccountService::CustomFieldValue, :count).from(0)
        end
        ...
        context 'with existing value' do
          ...
          it 'destroys existing custom field value' do
            expect { response }.to change(AccountService::CustomFieldValue, :count).from(1).to(0)
          end
        end

        context 'when required' do
          let(:field) { create(:'account_service/custom_text_field', required: true) }
        ...
        context 'when required and previously set' do
          ...
          it 'does not create a custom field value' do
            expect { response }.not_to change(AccountService::CustomFieldValue, :count).from(1)
          end
        end
      end
    ...
    context 'with select field' do
      let!(:field) do
        create(:'account_service/custom_select_field', values: %w[0 1 2], default_values: %w[0])
      end
      ...
      describe 'event notify' do
        ...
        it 'publishes an event' do
          expect(Msgr).to receive(:publish) do |payload, opts|
            ...
            expect(payload).to eq AccountService::ProfileDecorator.new([field], user).as_event
          end
    ...
    context 'with custom validator' do
      before do
        cls = Class.new(AccountService::XikoloValidators::Base) do
      ...
      context 'with valid value' do
        ...
        it 'saves value in database' do
          expect { response }.to change {
            AccountService::CustomFieldValue.where(context: user, custom_field: record).size
          }.from(0).to(1)
        end
