...
describe AccountService::API::PasswordResetsController, type: :controller do
  ...
  describe '#create' do
    ...
    context 'with existing email address' do
      ...
      it 'creates new password reset' do
        expect { response }.to change(AccountService::PasswordReset, :count).from(0).to(1)
      end
    ...
    context 'with non-existing email address' do
      ...
      it 'does not create a password reset' do
        expect { response }.not_to change(AccountService::PasswordReset, :count)
      end
  ...
  describe '#show' do
    ...
    let(:reset) { create(:'account_service/password_reset') }
  ...
  describe '#update' do
    ...
    let(:user) { create(:'account_service/user') }
    let(:reset) { create(:'account_service/password_reset', user:) }
    ...
    context 'with valid password' do
      ...
      it 'removes reset token after use' do
        expect { response }.to \
          change(AccountService::PasswordReset.where(id: reset.id), :count)
          .from(1)
          .to(0)
      end
