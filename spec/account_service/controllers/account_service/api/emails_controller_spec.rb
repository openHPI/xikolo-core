...
describe AccountService::API::EmailsController, type: :controller do
  include_context 'account_service API controller'
  let(:user)  { create(:'account_service/user') }
  let(:email) { create(:'account_service/email', user:).reload }
  let(:params) { {} }
  ...
  describe '#update' do
    ...
    context 'with changed confirmation state' do
      let(:email) { create(:'account_service/email', user:, confirmed: false) }
    ...
    context 'make primary' do
      ...
      context 'with confirmed email' do
        let(:email) { create(:'account_service/email', user:, confirmed: true) }
    ...
    context 'make unconfirmed' do
      ...
      context 'on primary address' do
        let(:email) do
          create(:'account_service/email', user:, confirmed: true, primary: true)
        end
  ...
  describe '#create' do
    ...
    context 'with valid params' do
      ...
      it { expect { response }.to change(AccountService::Email, :count).by(1) }
    ...
    context 'with invalid email' do
      ...
      it { expect { response }.not_to change(AccountService::Email, :count) }
  ...
  describe '#destroy' do
    ...
    it 'deletes email record from database' do
      expect { response }.to change(AccountService::Email, :count).from(2).to(1)
    end
    ...
    context 'with primary address' do
      ...
      it 'does not delete database record' do
        expect { response }.not_to change(AccountService::Email, :count)
      end
