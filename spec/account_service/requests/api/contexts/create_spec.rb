...
describe 'Context: Creation', type: :request do
  ...
  it 'responds with a follow location to the created resource' do
    expect(resource.follow).to eq account_service.context_url(AccountService::Context.non_root.last)
  end

  it 'creates a database record' do
    expect { resource }.to change { AccountService::Context.non_root.count }.from(0).to(1)
  end

  it 'returns the created group' do
    expect(resource).to eq json(AccountService::Context.non_root.last)
  end

  it 'inherits from root context' do
    expect(resource['parent_id']).to eq AccountService::Context.root_id
  end
  ...
  context 'with reference' do
    ...
    it 'stored given reference' do
      resource
      expect(AccountService::Context.non_root.last.reference_uri).to eq 'urn:ietf:rfc:8141'
    end
