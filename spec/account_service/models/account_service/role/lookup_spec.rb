...
describe AccountService::Role, '.lookup', type: :model do
  ...
  describe '.lookup' do
    subject(:results) do
      AccountService::Role.lookup(principal:, context: request_context)
    end
    ...
    context 'with user principal' do
      ...
      context 'with multiple group memberships on different' do
        ...
        before do
          create(:'account_service/grant',
            principal: groups[1], role: roles[2], context: grant_context)
          create(:'account_service/grant',
            principal: groups[2], role: roles[3], context: AccountService::Context.root)
      ...
      context 'with special group grant' do
        ...
        context 'all users group' do
          let(:group) { AccountService::Group.all_users }

          it_behaves_like 'context:inheritance'
        end

        context 'active users group' do
          let(:group) { AccountService::Group.active_users }

          it_behaves_like 'context:inheritance'
        end

        context 'confirmed users group' do
          let(:group) { AccountService::Group.confirmed_users }

          it_behaves_like 'context:inheritance'
        end

        context 'unconfirmed users group' do
          let(:group) { AccountService::Group.unconfirmed_users }
        ...
        context 'archived users group' do
          let(:group) { AccountService::Group.archived_users }
