...
describe CourseService::ItemsController, type: :controller do
  ...
  describe '#index' do
      ...
      describe 'response' do
        ...
        describe 'json' do
          it 'contains the correct item resource' do
            expect(json).to contain_exactly(CourseService::ItemDecorator.new(item, context: {collection: true}).as_json(api_version: 1))
          end
        end
      end
    end
    ...
    describe '^filter' do
      context 'with section_id param set' do
        ...
        let(:params) { {section_id: section.id, format: 'json'} }
      ...
      context 'with section_id param set as array' do
        let(:section) { create(:'course_service/section') }
        let(:item) { create(:'course_service/item', section:) }
        let(:params) { {section_id: ['6551282f-b22f-43dd-855c-2860f560f54e', section.id], format: 'json'} }
      ...
      context 'with item from all sections only' do
        ...
        let(:params) { {available: false, course_id: section.course_id, format: 'json'} }
      ...
      context 'with item from available sections only' do
        ...
        let(:params) { {available: true, course_id: section.course_id, format: 'json'} }
      ...
      context 'with published item' do
        ...
        let(:params) { {published: true, format: 'json'} }
      ...
      context 'with featured item' do
        context 'with published item' do
          ...
          let(:params) { {featured: true, format: 'json'} }
      ...
      context 'with open_mode' do
        ...
        context 'with published item' do
          let(:params) { {course_id: course.id, open_mode: true, format: 'json'} }
        ...
        context 'without course_id' do
          let(:params) { {open_mode: true, format: 'json'} }
      ...
      context 'with available' do
        ...
        let(:params) { {course_id: course.id, available: true, format: 'json'} }
      ...
      context 'with was_available' do
        ...
        let(:params) { {course_id: course.id, was_available: true, format: 'json'} }
      ...
      context 'with course_id param set' do
        ...
        let(:params) { {course_id: course.id, format: 'json'} }
      ...
      context 'with content type filter set' do
        ...
        let(:params) { {content_type: 'quiz', format: 'json'} }
      ...
      context 'with exercise type filter set' do
        ...
        let(:params) { {exercise_type: 'main', format: 'json'} }
      ...
      context 'with multiple exercise type filter set as array' do
        ...
        let(:params) { {exercise_type: %w[main bonus], format: 'json'} }
      ...
      context 'with multiple exercise type filter set as comma-separated list' do
        let!(:main_items)  { create_list(:'course_service/item', 2, content_type: 'quiz', exercise_type: 'main') }
        let!(:bonus_items) { create_list(:'course_service/item', 3, content_type: 'quiz', exercise_type: 'bonus') }
        let(:params) { {exercise_type: 'main,bonus', format: 'json'} }
      ...
      context 'with multiple exercise type filter set' do
        let!(:main_items)  { create_list(:'course_service/item', 2, content_type: 'quiz', exercise_type: 'main') }
        let!(:bonus_items) { create_list(:'course_service/item', 3, content_type: 'quiz', exercise_type: 'bonus') }
        let(:params) { {exercise_type: {'0' => 'main', '1' => 'bonus'}, format: 'json'} }
    ...
    context 'with UUID' do
      ...
      let(:video_items) { create_list(:'course_service/item', 4, content_type: 'video') }
      let(:params) { {id: video_items[2].id, format: 'json'} }
      let(:action) { -> { get :index, params: } }
    ...
    context 'with new_for' do
      ...
      let(:params) { {new_for: user_id, format: 'json'} }
  ...
  describe '#show' do
    let(:action) { -> { get :show, params: {id: item.id, format: 'json'} } }
    ...
    context 'json' do
      ...
      it { is_expected.to eq CourseService::ItemDecorator.new(item).as_json(api_version: 1).stringify_keys }
    end
    ...
    context 'with user id' do
      ...
      let(:action) { -> { get :show, params: {id: item.id, for_user: user_id, format: 'json'} } }
  ...
  describe '#create' do
    let(:section) { create(:'course_service/section') }
    let(:params) { attributes_for(:'course_service/item').merge(section_id: section.id, format: 'json') }
    ...
    it 'creates new item' do
      expect { action.call }.to change(CourseService::Item, :count).from(0).to(1)
    end
    ...
    context 'with invalid data' do
      let(:params) { {title: 'test', format: 'json'} }
  ...
  describe '#update' do
    ...
    let(:action) { -> { patch :update, params: params.merge(id: item.id, format: 'json') } }
    ...
    context 'with not round float value' do
      ...
      let(:params) { {max_points: 2.13, format: 'json'} }
    ...
    context 'with negative value' do
      ...
      let(:params) { {max_points: -2, format: 'json'} }
    ...
    context 'position update' do
      ...
      context 'move to top' do
        let(:item) { items[3] }
        let(:params) { {position: 1, format: 'json'} }
      ...
      context 'move up' do
        let(:item) { items[5] }
        let(:params) { {position: 3, format: 'json'} }
      ...
      context 'move down' do
        let(:item) { items[2] }
        let(:params) { {position: 4, format: 'json'} }
      ...
      context 'move to bottom' do
        let(:item) { items[3] }
        let(:params) { {position: 7, format: 'json'} }
  ...
  describe 'with versioning', :versioning do
    ...
    let(:update_title) { -> { put :update, params: {id: item.id, title: 'Welcome', format: 'json'} } }
    ...
    context 'with given timestamp' do
      it 'show action should return version of item at this time' do
        ...
        item.reload
        expect(item.title).to eq 'Welcome'
        params = {id: item.id, version_at: DateTime.new(2009, 9, 1, 12, 0, 0).to_s, format: 'json'}
        get(:show, params:)
  ...
  describe '#current' do
    ...
    let(:action) { get :current, params: {course: course.id, user: user_id, format: :json} }
    ...
    context 'with only one section' do
      ...
      context 'as a previewer' do
        let(:action) { get :current, params: {course: course.id, user: user_id, preview: 'true', format: :json} }
    ...
    context 'with forks (and branches)' do
      ...
      context "with the first (resume) item not belonging to the user's branch" do
        before do
          ...
          items = CourseService::Structure::UserItemsSelector.new(course.node, user_id).items
