...
describe CourseService::StatisticsController, type: :controller do
  include_context 'course_service API controller'
  describe "GET 'show'" do
    subject { json }

    let(:json) { request.call; JSON.parse response.body }
    let(:request) { -> { get :show, params: {course_id: students.first.course_id, format: :json} } }
  ...
  describe "GET 'enrollment_stats'" do
    ...
    context 'global stats' do
      let(:params) { {start_date: 2.days.ago.to_s, end_date: DateTime.now.to_s, format: :json} }
    ...
    context 'filter by classifier' do
      let(:params) { {start_date: 2.days.ago.to_s, end_date: DateTime.now.to_s, classifier_id: CourseService::Classifier.find_by(title: 'internet').id, format: :json} }
