...
describe CourseService::LastVisitsController, type: :controller do
  ...
  describe '#show' do
    context 'without user' do
      let(:action) { get :show, params: {course_id: course.id, format: :json} }

      its(:status) { is_expected.to eq 404 }
    end

    context 'without enrollment' do
      let(:action) { get :show, params: {course_id: course.id, user_id:, format: :json} }
