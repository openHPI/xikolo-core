...
describe CourseService::SectionsController, type: :controller do
  ...
  describe "GET 'index'" do
    ...
    it 'answers with section resources' do
      get :index
      expect(json[0]).to eq CourseService::SectionDecorator.new(section).as_json(api_version: 1).stringify_keys
    end
  ...
  describe 'GET \'show\'' do
    ...
    context 'json' do
      ...
      it 'responds with sections' do
        expect(json).to eq(CourseService::SectionDecorator.new(section).as_json(api_version: 1).stringify_keys)
      end
    end
  end
  ...
  describe 'POST \'create\'' do
    ...
    it 'creates a section on create' do
      expect { post(:create, params:) }.to change(CourseService::Section, :count).from(1).to(2)
    end
  ...
  describe "DELETE 'destroy'" do
    ...
    it 'deletes the section' do
      expect { action.call }.to change { list_action.call; JSON[response.body].size }.from(1).to(0)
        .and change(CourseService::Section, :count).from(1).to(0)
    end
    ...
    context 'with items' do
      ...
      it 'does not change the section count' do
        expect { action }.not_to change(CourseService::Section, :count)
      end
    ...
    context 'parent with children' do
      ...
      it 'removes the section and its child sections' do
        expect { action.call }.to change { list_action.call; JSON[response.body].size }.from(1).to(0)
          .and change(CourseService::Section, :count).from(10).to(4)
      end
    end
  end
end
