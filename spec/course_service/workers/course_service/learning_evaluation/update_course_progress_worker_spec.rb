...
RSpec.describe CourseService::LearningEvaluation::UpdateCourseProgressWorker, type: :worker do
  subject(:perform) do
  ...
  context 'without persisted_learning_evaluation config' do
    ...
    describe '#perform' do
      it 'generates no course progress' do
        expect { perform }.not_to change(CourseService::CourseProgress, :count)
      end
    end
  end
  ...
  context 'with persisted_learning_evaluation config' do
    ...
    describe '#perform' do
      it 'generates a course progress' do
        expect { perform }.to change(CourseService::CourseProgress, :count).from(0).to(1)
      end
    end
  end
end
