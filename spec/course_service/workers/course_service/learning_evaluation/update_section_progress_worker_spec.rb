...
RSpec.describe CourseService::LearningEvaluation::UpdateSectionProgressWorker, type: :worker do
  subject(:perform) do
  ...
  context 'without persisted_learning_evaluation config' do
    ...
    describe '#perform' do
      it 'generates no section progress' do
        expect { perform }.not_to change(CourseService::SectionProgress, :count)
      end
    end
  end
  ...
  context 'with persisted_learning_evaluation config' do
    ...
    describe '#perform' do
      it 'generates a section progress' do
        expect { perform }.to change(CourseService::SectionProgress, :count).from(0).to(1)
      end
    end
  end
end
