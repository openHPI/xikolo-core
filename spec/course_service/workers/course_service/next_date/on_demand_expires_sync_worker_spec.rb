...
RSpec.describe CourseService::NextDate::OnDemandExpiresSyncWorker, type: :worker do
  ...
  let!(:other_next_date) { CourseService::NextDate.create! attrs.merge(date: 2.days.from_now, user_id: other_enrollment.user_id) }
  ...
  context 'for enrollment with forced_submission_date' do
    ...
    it 'creates an next date' do
      ...
      expect(CourseService::NextDate.count).to eq 2
      date = CourseService::NextDate.where.not(user_id: other_next_date.user_id).first
  ...
  context 'for enrollment without forced_submission_date' do
    ...
    it 'removes an previous date' do
      ...
      expect { execute }.to change(CourseService::NextDate, :count).from(2).to(1)
    end
