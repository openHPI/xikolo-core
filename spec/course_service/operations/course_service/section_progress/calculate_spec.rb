...
describe CourseService::SectionProgress::Calculate, type: :operation do
  ...
  it 'creates / updates the section progress and triggers a recalculation' do
    ...
    CourseService::SectionProgress.first.update!(visits: 0, main_dpoints: 0, main_exercises: 0)

    expect { calculate }.not_to change(CourseService::SectionProgress, :count).from(1)
    expect(CourseService::SectionProgress.first).to have_attributes(
  ...
  it 'creates / updates the corresponding course progress' do
    ...
    CourseService::CourseProgress.first.update!(visits: 0, main_dpoints: 0, main_exercises: 0, max_dpoints: 0, max_visits: 0)

    expect { calculate }.not_to change(CourseService::CourseProgress, :count).from(1)
    expect(CourseService::CourseProgress.first).to have_attributes(
  ...
  context 'when explicitly skipping the course progress update' do
    ...
    it 'does not create / update the corresponding course progress' do
      # CourseService::Creating a visit / result already creates a course progress, reset it.
      CourseService::CourseProgress.first.update!(visits: 0, main_dpoints: 0, main_exercises: 0, max_dpoints: 0, max_visits: 0)
  ...
  context 'with stale date' do
    ...
    it 'creates *and calculates* the section progress if none exists' do
      # Creating a visit / result already creates a course progress, reset it.
      CourseService::SectionProgress.first.destroy

      expect { calculate }.to change(CourseService::SectionProgress, :count).from(0).to(1)
      expect(CourseService::SectionProgress.first).to have_attributes(
        section_id: section.id,
        user_id:,
        visits: 1,
    ...
    context 'with outdated section progress' do
      let(:section_progress) { CourseService::SectionProgress.first }
