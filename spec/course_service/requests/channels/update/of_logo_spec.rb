  ...
  context 'with valid upload' do
    let!(:store_stub) do
      stub_request(:put, %r{https://s3.xikolo.de/xikolo-public/
        channels/[0-9a-zA-Z]+/logo_v2.jpg}x).to_return(status: 200, body: '<xml></xml>')
    end
    ...
    it 'stores the logo' do
      expect { action }.to change { channel.reload.logo_uri }
    ...
    it 'schedules removal of the old file' do
      ...
      expect(CourseService::FileDeletionWorker.jobs.last['args']).to eq [
        's3://xikolo-public/channels/1/logo_v1.jpg',
      ]
    end
  end
