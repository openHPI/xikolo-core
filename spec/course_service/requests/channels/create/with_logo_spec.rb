...
describe 'Channel: create with logo', type: :request do
  ...
  context 'with valid upload' do
    let!(:store_stub) do
      stub_request(:put, %r{https://s3.xikolo.de/xikolo-public/
        channels/[0-9a-zA-Z]+/logo_v1.jpg}x).to_return(status: 200, body: '<xml></xml>')
    end
    ...
    before do
      ...
      stub_request(:head, file_url).to_return(
        ...
        headers: {
          ...
          'X-Amz-Meta-Xikolo-Purpose' => 'course_channel_logo',
  ...
  context 'with invalid file upload' do
    ...
    it 'handles S3 errors during upload copying' do
      ...
      stub_request(:put, %r{https://s3.xikolo.de/xikolo-public/
        channels/[0-9a-zA-Z]+/logo_v1.jpg}x).to_return(status: 403)
