...
describe 'Items: List', type: :request do
    ...
    context 'with a valid user ID' do
      ...
      it_behaves_like 'an item list with forks and branches' do
        ...
        let(:branch1_assertions) do
          ...
          expect(list.select {|row| row['user_visit'].nil? }).to have(5).items # rubocop:disable RSpec/ExpectInLet
          expect(list.map(&:to_hash).select {|row| row['user_visit'].nil? }).to match([ # rubocop:disable RSpec/ExpectInLet
            a_hash_including('id' => items.third.id),
            a_hash_including('id' => items.fourth.id),
          ...
          expect(list.select {|row| row['user_visit'].respond_to? :to_hash }).to have(2).items # rubocop:disable RSpec/ExpectInLet
          expect(list.map(&:to_hash).select {|row| row['user_visit'].respond_to? :to_hash }).to match([ # rubocop:disable RSpec/ExpectInLet
            a_hash_including('id' => items.first.id),
            a_hash_including('id' => items.second.id),
          ])
        end
        ...
        let(:branch2_assertions) do
          ...
          expect(list.select {|row| row['user_visit'].nil? }).to have(4).items # rubocop:disable RSpec/ExpectInLet
          expect(list.map(&:to_hash).select {|row| row['user_visit'].nil? }).to match([ # rubocop:disable RSpec/ExpectInLet
            a_hash_including('id' => items.third.id),
            a_hash_including('id' => items.fourth.id),
            a_hash_including('id' => item1.id),
  ...
  describe 'state_for' do
    ...
    it_behaves_like 'an item list with forks and branches' do
      let(:branch1_assertions) do
        expect(list.map(&:to_hash)).to all(include('user_state' => 'new')) # rubocop:disable RSpec/ExpectInLet
      end
      let(:branch2_assertions) do
        expect(list.map(&:to_hash)).to all(include('user_state' => 'new')) # rubocop:disable RSpec/ExpectInLet
      end
