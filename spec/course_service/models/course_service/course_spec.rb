...
describe CourseService::Course, type: :model do
  ...
  context '(enrollment completion worker)' do
    ...
    it 'does not start worker when records_released disabled and thresholds change' do
      expect do
        ...
        course.cop_threshold_percentage = 50
        course.save
      end.not_to change(CourseService::EnrollmentCompletionWorker.jobs, :size)
    end
  ...
  context 'scopes' do
    describe '#by_identifier' do
      subject { CourseService::Course.by_identifier(identifier).take! }
    ...
    describe 'current' do
      subject { CourseService::Course.current.pluck :id }
    ...
    describe 'for_user' do
      subject(:courses) { CourseService::Course.for_user(current_user) }
  ...
  describe 'certificates' do
    ...
    describe 'confirmation_of_participation?' do
      ...
      context 'with released records and cop enabled' do
        ...
        # TODO: Test should test implemented methods.
        around do |examples|
