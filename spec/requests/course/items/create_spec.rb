...
describe 'Course: Items: Create', type: :request do
  ...
  context 'for logged-in user' do
    ...
    context 'for peer assessments' do
      let(:item) do
        build(:'course:item', content_type: 'peer_assessment', open_mode: false)
      end
      let(:params) do
        {
          xikolo_course_item: {
          ...
          existing_assessment: 'new',
        }
      end
      let(:content_create_stub) do
        Stub.request(
          :peerassessment, :post, '/peer_assessments',
          body: hash_including(
            title: item['title'],
            course_id: course['id']
          )
      ...
      let(:content_update_stub) do
        Stub.request(
          :peerassessment, :put, "/peer_assessments/#{content_id}",
          body: hash_including(
            title: item['title'],
            course_id: course['id'],
            item_id: item['id']
      ...
      let(:content_delete_stub) do
        Stub.request(:peerassessment, :delete, "/peer_assessments/#{content_id}")
          .to_return Stub.response(status: 200)
      end
      ...
      context 'properly created content and item resources' do
        ...
        it 'creates the item and its content resource and properly adjusts the redirect' do
          ...
          # The created peer assessment content resource is properly updated
          # with the item resource ID
          expect(content_update_stub).to have_been_requested.once

          # The redirect path is modified for quiz items
          expect(response).to redirect_to "/peer_assessments/#{UUID4(content_id).to_s(format: :base62)}/edit"
        end
      end
      ...
      context 'with error while persisting the content resource' do
        ...
        it 'does not try to update a missing (i.e. not created) content resource' do
          create_item

          expect(content_update_stub).not_to have_been_requested
        end
      end
      ...
      context 'with existing assignment' do
        let(:params) do
          {
            ...
            existing_assessment: content_id,
          }
        end
        let(:content_show_stub) do
          Stub.request(:peerassessment, :get, "/peer_assessments/#{content_id}")
            .to_return Stub.json({
              id: content_id,
        ...
        let(:existing_item_id) { SecureRandom.uuid }
        let(:content_update_stub) do
          Stub.request(
            :peerassessment, :put, "/peer_assessments/#{content_id}",
            body: hash_including(
        ...
        it 'properly creates the item resource (copying over the content ID)' do
          create_item

          # Loads already existing content resource for the peer assessment
          expect(content_show_stub).to have_been_requested.once
          # As the peer assessment content resource exists for another item,
          # the item ID is not updated.
          expect(content_update_stub).to have_been_requested.once
          expect(content_delete_stub).not_to have_been_requested
          ...
          # The redirect path is not modified in this case
          expect(response).to redirect_to "/courses/#{course['course_code']}/sections"
        end
      end
    end
