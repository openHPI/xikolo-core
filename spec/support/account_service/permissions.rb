...
RSpec.shared_examples 'shared:permissions' do
  ...
  describe 'all user grants' do
    before do
      create(:'account_service/grant', principal: AccountService::Group.all_users, role: roles[3])
    end
  ...
  describe 'active user grants' do
    before do
      create(:'account_service/grant', principal: AccountService::Group.active_users, role: roles[3])
    end
  ...
  describe 'confirmed user grants' do
    before do
      create(:'account_service/grant', principal: AccountService::Group.confirmed_users, role: roles[3])
    end
