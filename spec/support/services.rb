...
RSpec.configure do |config|
  config.before(:each) do
    ...
    stub_request(:any, /account_service|quiz_service|pinboard_service|notification_service|course_service|news_service/).to_rack(
      Rack::Builder.new do
...
RSpec.configure do |config|
  config.before(:each, type: :request) do |example|
    if example.metadata[:location].match?(/account_service|quiz_service|pinboard_service/)
      host! 'test.host'
    end
  end
end
