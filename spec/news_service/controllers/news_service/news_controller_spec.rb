...
def decorate(news)
  NewsService::NewsDecorator.new(news).as_json(api_version: 1).stringify_keys
end

describe NewsService::NewsController, type: :controller do
  routes { NewsService::Engine.routes }
  let!(:news) { create(:'news_service/news', :published) }
  ...
  describe 'GET index' do
    ...
    context 'for a course' do
      ...
      before do
        ...
        create(:'news_service/news', :global, :published)
        create(:'news_service/news', :published, course_id: other_course_id)
      end
    ...
    context 'for a user_id' do
      ...
      context 'when including global news' do
        ...
        let!(:global_news) { create(:'news_service/news', :global, :published) }
        let!(:restricted_global_news) { create(:'news_service/news', :global, :published, audience: 'xikolo.affiliated') }
    ...
    describe 'order' do
      context 'latest first' do
        let!(:news_list) { create_list(:'news_service/news', 4) }
        let!(:news) { create(:'news_service/news') }
    ...
    context 'with filter' do
      before do
        create_list(:'news_service/news', 5)

        # other course news:
        create(:'news_service/news', course_id: SecureRandom.uuid)

        # other restricted global news:
        create(:'news_service/news', :global, :published, audience: 'xikolo.affiliated')
      end

      let(:news) { create(:'news_service/news') }
      let!(:home_news) { create(:'news_service/news', show_on_homepage: true) }
      let!(:global_news) { create(:'news_service/news', course_id: nil) }
      let!(:global_home_news) { create(:'news_service/news', course_id: nil, show_on_homepage: true) }
      let!(:future_global_news) { create(:'news_service/news', :global, publish_at: 2.days.from_now) }
      let!(:published_global_news) { create(:'news_service/news', :global, publish_at: 2.days.ago) }
    ...
    context 'global read count' do
      ...
      let!(:news) { create(:'news_service/news', :published, :read) }
      ...
      it 'answers with news resources' do
        index
        expect(json[0]).to eq(NewsService::NewsDecorator.new(news.reload).as_json(api_version: 1).stringify_keys.merge!('read_count' => 1))
      end
    end
  end
end
