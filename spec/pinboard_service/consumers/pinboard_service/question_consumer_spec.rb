...
describe PinboardService::QuestionConsumer, type: :consumer do
  let(:consumer)    { PinboardService::QuestionConsumer.new }
  ...
  describe '#read_question' do
    context 'first time' do
      it 'creates a new Watch' do
        expect { consumer.read_question }.to change(PinboardService::Watch, :count).from(0).to(1)
      end
    end
