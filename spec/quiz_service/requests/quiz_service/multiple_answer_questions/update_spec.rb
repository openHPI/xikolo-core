...
RSpec.describe 'Multiple Answer Questions: Update', type: :request do
  ...
  context 'when changing the question type' do
    ...
    let(:new_type) { 'QuizService::MultipleChoiceQuestion' }

    it 'updates the question\'s type' do
      resource
      expect(QuizService::Question.first.type).to eql new_type
    end

    context 'with too many correct answers' do
      before do
        create_list(:'quiz_service/answer', 2, question:, correct: true)
      end
