...
RSpec.describe 'Multiple Choice Questions: Create', type: :request do
  subject(:resource) { api.rel(:multiple_choice_questions).post(payload).value! }

  let(:api) { restify_with_headers(quiz_service_url).get.value! }

  let(:payload) { attributes_for(:'quiz_service/multiple_choice_question', quiz_id: quiz.id) }
  ...
  it 'creates a new multiple choice question' do
    expect { resource }.to change(QuizService::MultipleChoiceQuestion, :count).from(0).to(1)
  end
