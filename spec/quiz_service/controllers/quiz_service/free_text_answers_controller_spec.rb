...
describe QuizService::FreeTextAnswersController, type: :controller do
  include_context 'quiz_service API controller'

  let(:answer) { create(:'quiz_service/free_text_answer') }
  let(:params) { attributes_for(:'quiz_service/free_text_answer', type: 'FreeTextAnswer') }
  ...
  describe '#create' do
    ...
    context 'FreeTextAnswer' do
      ...
      let(:params) { attributes_for(:'quiz_service/answer').merge(correct: false, question_id: question.id) }
  ...
  describe '#destroy' do
    ...
    let!(:answer) { create(:'quiz_service/free_text_answer', text: 's3://xikolo-quiz/quizzes/1/1/test.jpg') }
  ...
  describe 'with versioning', :versioning do
    let(:answer1) { create(:'quiz_service/free_text_answer', correct: true, comment: 'Correct answer, well done!') }
    let(:answer2) { create(:'quiz_service/free_text_answer', correct: true) }
    let(:answer3) { create(:'quiz_service/free_text_answer', correct: false) }
