...
describe QuizService::XmlImporter::Quiz do
  ...
  context 'with valid xml' do
    ...
    context 'without external references' do
      ...
      it 'creates two questions' do
        expect { create_quizzes }.to change(QuizService::Question, :count).from(0).to(2)
      end

      it 'creates three answers' do
        expect { create_quizzes }.to change(QuizService::Answer, :count).from(0).to(3)
      end
      ...
      it 'stores a fallback text to avoid empty quiz instructions' do
        create_quizzes
        expect(QuizService::Quiz.first.instructions).to eq 'INSERT INSTRUCTION HERE'
      end
      ...
      context 'with not matching course code' do
        ...
        it 'raises parameter error' do
          expect { create_quizzes }.to raise_error(QuizService::XmlImporter::ParameterError)
        end
      end
    ...
    context 'with external references' do
      ...
      context 'with one existing & one new quiz' do
        let(:quiz) { create(:'quiz_service/quiz', external_ref_id: quiz_1_ref) }
        let(:question_1) do
        ...
        before do
          create(:'quiz_service/answer', question: question_1)
          create(:'quiz_service/answer', question: question_1)
          create(:'quiz_service/answer', question: question_2)
          create(:'quiz_service/answer', question: question_2)
        end

        it 'creates one quiz' do
          expect { create_quizzes }.to change(QuizService::Quiz, :count).from(1).to(2)
        end

        it 'creates the correct quiz' do
          create_quizzes
          expect(QuizService::Quiz.find_by(external_ref_id: quiz_2_ref)).to be_truthy
        end

        it 'creates two questions' do
          expect { create_quizzes }.to change(QuizService::Question, :count).from(2).to(4)
        end

        it 'creates three answers' do
          expect { create_quizzes }.to change(QuizService::Answer, :count).from(4).to(7)
        end
        ...
        it 'stores a fallback text to avoid empty quiz instructions' do
          create_quizzes
          expect(QuizService::Quiz.find_by(external_ref_id: quiz_2_ref).instructions).to eq 'INSERT INSTRUCTION HERE'
        end
      end
      ...
      context 'with an existing quiz with the same external_ref_id in the same course' do
        ...
        it 'does not create a new quiz' do
          expect { create_quizzes }.not_to change(QuizService::Quiz, :count).from(1)
        end
      end
      ...
      context 'with an existing quiz with the same external_ref_id in a another course' do
        ...
        it 'creates both quizzes' do
          quiz
          expect { create_quizzes }.to change(QuizService::Quiz, :count).from(1).to(3)
        end
      end
    end
  end
  ...
  context 'with invalid XML' do
    ...
    it 'raises schema error' do
      expect { create_quizzes }.to raise_error(QuizService::XmlImporter::SchemaError)
    end
  end
end
