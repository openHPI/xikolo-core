...
class Course < ApplicationRecord # rubocop:disable Layout/IndentationWidth
  self.table_name = :courses

  include ForGroups
  include CourseProviderSync

  has_one :node, class_name: 'CourseService::Structure::Root', foreign_key: :course_id, dependent: :destroy # rubocop:disable Rails/RedundantForeignKey
  has_one :visual, class_name: 'CourseService::Duplicated::Visual', dependent: :destroy
  ...
  has_many :relations,
    ...
    class_name: 'CourseService::Relation'
  has_many :content_tests
  has_many :offers, class_name: 'CourseService::Duplicated::Offer', dependent: :destroy

  has_many :course_documents, dependent: :delete_all
