...
class API::V2::Course::CoursesController < API::V2::RootController # rubocop:disable Layout/IndentationWidth
  ...
  def collection
    courses = CourseService::Course.from('embed_courses AS courses').not_deleted

    courses = apply_channel_filter(courses) if params[:channel]

    if params[:document_id]
      courses = courses.joins(:course_documents)
        .merge(CourseDocument.where(document_id: params[:document_id]))
    end
