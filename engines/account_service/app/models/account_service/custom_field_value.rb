...
class CustomFieldValue < ApplicationRecord # rubocop:disable Layout/IndentationWidth
  ...
  class << self
    def for_members_of(group)
      where(context_type: User.name, context_id: group.member_ids_relation)
    end

    def histograms(fields = CustomField.where(type: CustomSelectField.name))
