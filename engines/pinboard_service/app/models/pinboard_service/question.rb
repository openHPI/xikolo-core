...
class Question < ApplicationRecord # rubocop:disable Layout/IndentationWidth
  ...
  belongs_to :accepted_answer, class_name: 'PinboardService::Answer', optional: true
  has_many :answers
  has_many :public_answers, -> { unblocked.undeleted }, class_name: 'PinboardService::Answer'
  ...
  has_many :public_comments, -> { unblocked.undeleted }, as: :commentable, class_name: 'PinboardService::Comment'
  has_many :votes, as: :votable
  has_one  :requested_user_vote,
    -> { where 'votes.user_id' => Thread.current[:requested_user_id] },
    as: :votable,
    class_name: 'PinboardService::Vote'
  has_many :subscriptions, dependent: :destroy
  has_many :watches
  has_one  :user_watch,
    -> { where 'watches.user_id' => Thread.current[:requested_user_id] },
    class_name: 'PinboardService::Watch'
  ...
  def num_replies
    public_answers.count + public_comments.count + public_answer_comments.count
  end
