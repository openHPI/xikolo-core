...
module Question::Search # rubocop:disable Layout/IndentationWidth
    ...
    WITH
      ...
      public_comments AS (
        ...
        WHERE deleted = false
          AND workflow_state NOT IN ('blocked', 'auto_blocked')
          AND (
            (commentable_type = 'PinboardService::Question' AND commentable_id = $1) OR
            (commentable_type = 'PinboardService::Answer' AND commentable_id IN (SELECT id FROM public_answers))
          )
      )
  ...
  class_methods do
    ...
    def schedule_search_text_update
      pluck(:id).each {|id| PinboardService::UpdateQuestionSearchTextWorker.perform_async(id) }
    end
  end

  def schedule_search_text_update
    PinboardService::UpdateQuestionSearchTextWorker.perform_async(id)
  end
  ...
  rescue PinboardService::Course::NotFound # The course does not exist (anymore?)
    nil
  end
end
end
