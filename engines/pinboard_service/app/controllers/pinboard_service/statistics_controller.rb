  ...
  def most_active_users(count, with_teaching_team: false)
    ...
    sql = <<~SQL.squish
      SELECT user_id, COUNT(user_id) AS count FROM
        ...
        UNION ALL SELECT c.user_id FROM comments c JOIN questions q ON c.commentable_id = q.id WHERE c.commentable_type = 'PinboardService::Question' AND q.course_id = :course_id
        UNION ALL SELECT c.user_id FROM comments c JOIN answers a ON c.commentable_id = a.id JOIN questions q ON a.question_id = q.id WHERE c.commentable_type = 'PinboardService::Answer' AND q.course_id = :course_id) x
