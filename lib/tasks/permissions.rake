    ...
    ActiveRecord::Base.transaction do
      Rails.root.glob('lib/tasks/permissions/*.{yaml,yml}').sort.each do |file|
        ...
        data.fetch('groups', []).each do |group|
          ...
          AccountService::Group.find_or_initialize_by(name: group['name']).update!(group)
        end

        data.fetch('roles', {}).each do |name, permissions|
          puts "Syncing role #{name}..."
          AccountService::Role.find_or_initialize_by(name:).update!(permissions:)
        end

        data.fetch('grants', []).each do |grant|
          puts "Syncing grant #{grant}..."

          role = AccountService::Role.resolve(grant['role'])
          group = AccountService::Group.resolve(grant['group'])
          context = AccountService::Context.resolve(grant.fetch('context', 'root'))

          AccountService::Grant.find_or_create_by!(role:, principal: group, context:)
        end
      end
      ...
      OBSOLETE_ROLES.each do |role|
        ...
        AccountService::Role.find_by(name: role)&.destroy
      end
    end
  end
end
